...
RUN yum update -y
RUN yum install -y unzip cmake make python34 python34-setuptools python34-pip
...
RUN yum install -y libtool libusbx-devel libusb-devel librtlsdr librtlsdr-devel rtl-sdr rtl-sdr-devel
RUN cmake ../
RUN make
RUN make install
WORKDIR /opt
